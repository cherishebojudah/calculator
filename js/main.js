...
function reverseumberFormat(num) {
...
for (var i = 0; i < operator.length; i++) {
    operator[i].addEventListener('click', function(){
        if (this.id=='clear') {
            ...
            var output = reverseumberFormat(getOutput).toString();
            if(output) {
            ...
            if(output==''&& history!='') {
                ...
                if (output!='' || history!='') {
                output=output==''?output:reverseumberFormat(output);
...
for (var i = 0; i < number.length; i++) {
    number[i].addEventListener('click', function(){
        var output = reverseumberFormat(getOutput());
    if(output != Nan) 
